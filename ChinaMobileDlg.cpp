...
int laserReadingNo=0,laserReadingX[1024],laserReadingY[1024],pathReadingNo=0,pathReadingX[1024],pathReadingY[1024];
...
int taskGoal_green[2][1024], taskGoal_red[2][1024], taskGoal_yellow[2][1024];

//Status：返回状态，200：运行正常，500运行异常 
//Licensestate：证书状态, 100合法,404非法      
	...
	, m_speedDisplay(_T(""))
{
	...
	memcpy(taskID,"12316h",sizeof("12312a")); // taskID should less than 50 byte
}
			...
			heartBeat_count = heartBeat_count + 1;
				...
				if(heartBeat_status == 200)
				{
					if(heartBeat_state == "101") //就绪
					{
						...
						Sleep(1000);
						if(heartBeat_count == 10)goto heartBeat_out;
...
int CChinaMobileDlg::conLoginCloudReqBody(std::string &reqbody)
{
	...
	Json::Value item2;
		...
		item2["imsi"]="866513020002395";
		item2["phoneno"]="13889808194";
...
int CChinaMobileDlg::conRepTestStatusReqBody(std::string &reqbody)
{
	...
	Json::Value item2, item3;
		...
		//--- device 2---//
        item2["sn"]="866513020002395";    //device 2 appID
